...
async fn group_by_having() {
    ...
    assert_eq!(cat1, "B");
    assert_eq!(total1, 70);
    let cat2: String = rows[2].get("CAT");
    let cnt2: i64 = rows[2].get("CNT");
    let total2: i64 = rows[2].get("TOTAL");
    assert_eq!(cat2, "C");
    assert_eq!(cnt2, 1);
    assert_eq!(total2, 50);
...
async fn like_pattern() {
    ...
    assert_eq!(rows[2].get::<String, &str>("NAME"), "Dave");
...
async fn clob_string() {
    ...
    let row = sqlx::query("SELECT TO_CLOB(?) AS v FROM DUAL")
        .bind(&long_str)
        .fetch_one(&mut conn).await.unwrap();
    let v: String = row.get("V");
...
async fn struct_join() {
    let mut conn = new_conn().await;
    let _ = sqlx::query("DROP TABLE sqlx_test_j_dept2").execute(&mut conn).await;
    let _ = sqlx::query("DROP TABLE sqlx_test_j_emp2").execute(&mut conn).await;
