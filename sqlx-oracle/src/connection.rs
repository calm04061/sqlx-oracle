                        ...
                        for (i, col) in columns.iter().enumerate() {
                            ...
                            if is_null {
                                ...
                                let raw_bytes = match col.type_info {
                                    ...
                                    OracleTypeInfo::Clob | OracleTypeInfo::NClob => {
                                        let lob: sibyl::CLOB<'_> = sibyl_row.get(i).map_err(|e| {
                                            Error::Decode(
                                                format!("failed to get clob column {i}: {e}").into(),
                                            )
                                        })?;
                                        let len = lob.len().await.map_err(|e| {
                                            Error::Decode(
                                                format!("failed to get clob length column {i}: {e}").into(),
                                            )
                                        })?;
                                        let mut text = String::with_capacity(len);
                                        lob.read(0, len, &mut text).await.map_err(|e| {
                                            Error::Decode(
                                                format!("failed to read clob column {i}: {e}").into(),
                                            )
                                        })?;
                                        text.into_bytes()
                                    }
